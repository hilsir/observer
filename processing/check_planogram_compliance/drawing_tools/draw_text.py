...
class TextDrawer:
    def __init__(self, font_name="DejaVuSans-Bold.ttf", default_size=14):
        # Ищем корень проекта — поднимаемся вверх от этого файла, пока не найдём папку fonts/.
        # Не завязываемся на то, сколько уровней вложенности у этого файла — оно может меняться.
        project_root = Path(__file__).resolve().parent
        while not (project_root / "fonts").exists() and project_root != project_root.parent:
